...
rootPath = r"Z:\Project#_1300_1499\#1415 SM HE Keck CdTe LANL\set-SMK020FTR"

baseFileName = "50KV_0C_1ms_cap"
# backFile = file_select("Background File")
# foreFile = file_select("Foreground File")
cap = 0
backFile = rootPath +  f"\\run-{baseFileName}{cap}_b" +  r"\frames" + f"\\{baseFileName}{cap}_b_00000001.raw"
foreFile = rootPath +  f"\\run-{baseFileName}{cap}_f" +  r"\frames" + f"\\{baseFileName}{cap}_f_00000001.raw"
...
avgFore = foreStack/nImages

plotData = (avgFore-avgBack) * ffCorect
plotDataClip = np.clip(plotData, clipLow, clipHigh)
tavgplotData = gc.GeoCor(np.average(plotDataClip, axis=0))
W = 256 
H = 128 
avgplotData = tavgplotData[147:147+H, 261:261+W ]
